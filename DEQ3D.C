...
void __fastcall SaveBMPConverter( const char *filename, const void *data, const unsigned int w, const unsigned int h, const unsigned int bpp ){ SaveBMP( filename, (unsigned char*)data, w, h, bpp ); }
...
void __fastcall Proyected( const int *src, int *dst, unsigned int n, int fov ){
	...
	while(n--){
		...
		// z+=(z==0);
		fov = ffac / z;
		*dst++ = ( ( src[0] * fov | 2048 ) >> 12 ) + DW;
		*dst++ = DH - ( ( src[1] * fov | 2048 ) >> 12 );
		*dst++ = fov >> shift;
		src += 3;
	}
}
...
// /////////////////////////////////////////////////////////////////////////////////////////////////////////
// /////////////////////////////////////////////////////////////////////////////////////////////////////////
// TABLA DE DIVICIONES DE PUNTOFIJO SHIFT 16.

#define SHIFTDIV 15
static int TABLEDIV[SW];

void GenerarTablaDiv(void){
	TABLEDIV[0] = 1;
	for (int i = 1; i < SW; ++i)
	{
		TABLEDIV[ i ] = (1<<SHIFTDIV) / i;
	}
}
...
static EDGE_FLAT *SELECTOR;// <-- -->
static int MinY;
static int MaxY;
void ScanLine( int x0, int y0, int z0, int x1, int y1, int z1 ){
	unsigned int Len;
	SELECTOR = DR;
	if (y0>y1)
		y0^=y1, y1^=y0, y0^=y1,
	...
	z0<<=SHIFTDIV;
	while( Len-- ){
		// *SELECTOR++ = (EDGE_FLAT){ x0>>SHIFTDIV, z0 };
		SELECTOR->x = x0>>SHIFTDIV;
		SELECTOR->z = z0;
		SELECTOR++;
		x0+=x1;
		z0+=z1;
...
void __fastcall TriangleFill( unsigned int *P, int *B, int *buftri_, const unsigned int c ){
	unsigned int *VID;
	int *BUF;
	int z0;
	int z1;
	...
	while( MaxY-- )
	{
		if ( PL->x <= PR->x )
		{
			...
			PR->x -= PL->x;
			PR->z -= PL->z;
			z1 = (PR->z * TABLEDIV[PR->x])>>SHIFTDIV;
			z0 = PL->z;
			while( PR->x-- ){
				*VID = (*BUF>=z0?c:*VID);
				*BUF = (*BUF>=z0?z0:*BUF);
...
void __fastcall MallaRotarX( int *_3d, const int theta, unsigned int len )
{
	//Rotacion X
    int SIN,COS;
    SIN = tdSIN( theta );
...
void __fastcall MallaRotarY( int *_3d, const int theta, unsigned int len )
{
	// Rotacion Y
    int SIN,COS;
    SIN = tdSIN( theta );
...
void __fastcall MallaRotarZ( int *_3d, const int theta, unsigned int len )
{
	// Rotacion Z
    int SIN,COS;
    SIN = tdSIN( theta );
...
int main(int argc, char const *argv[])
{
	...
	// Si Extra.Fvm no existe puede activar esta linea y generara el cubo.
	{
		if (argc<2)
		{
			...
			printf("SCALE: ");
			scanf("%f",&ScaleZ);
			GenerateFvmToObj( &MALLA3D, argv[1], (int)ScaleZ );
			GenerateFvm( &MALLA3D, "Extra.Fvm" );
		}
	}
	GenerarTablaDiv();
	...
	// MallaRotarX( MALLA3D.V, 30, MALLA3D.NV );
	MallaRotarY( MALLA3D.V, 10, MALLA3D.NV );
